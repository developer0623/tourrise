...
RSpec.describe Easybill::CreateCustomerJob, type: :job do
  before do
  ...
  describe '.perform' do
    ...
    before do
      allow(Easybill::CreateCustomer).to receive(:call)
    end

    it 'calls the create customer interactor' do
      described_class.perform_now(customer_data)

      expect(Easybill::CreateCustomer).to have_received(:call).with(data: 'data')
    end
  end
end
