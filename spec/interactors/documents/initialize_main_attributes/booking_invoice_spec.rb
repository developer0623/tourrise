...
RSpec.describe Documents::InitializeMainAttributes::BookingInvoice, type: :interactor do
  ...
  let(:params) { { "payment_attributes" => payment_attributes, "description" => description } }
  let(:payment_attributes) { { "0" => { price_cents: 100, due_on: Time.zone.now } } }
  ...
  before do
    allow(BookingInvoice).to receive(:new) { document_double }
  end
