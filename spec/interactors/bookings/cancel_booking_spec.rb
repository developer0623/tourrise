...
RSpec.describe Bookings::CancelBooking, type: :interactor do
  describe '.call' do
    before do
      ...
      allow(Bookings::PublishBookingCanceledEvent).to receive(:call!) { true }
    end

    it 'calls the change booking state interactor with the right context' do
      context = described_class.call

      expect(context.transition_method_name).to eq(:cancel)
    end
  ...
  it 'organizes the correct interactors' do
    expected_organized_classes = [
      ...
      Bookings::PublishBookingCanceledEvent
    ]
